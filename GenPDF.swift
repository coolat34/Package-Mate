...
/// 1: Generate pdf from given view
         @MainActor
private func generatePDF(length: Double, width: Double, height: Double) -> URL {
        ///  1; Select UI View to render as pdf
    let renderer = ImageRenderer(content: DiagramContent(IM:IM))
                
        /// 2: Save it to our documents directory
        let url = URL.documentsDirectory.appending(path: "PackageMate.pdf")
    ...
    func sideCalc(tag: Int) -> Double {
        tag % 2 == 0 ? IM.WidthACT : IM.HeightACT
    }
} /// struct
